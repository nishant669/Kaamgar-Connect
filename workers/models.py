...
class WorkerProfile(models.Model):
    ...
    extra_skills = models.CharField(
        max_length=300,
        blank=True,
        help_text='Additional comma-separated skills'
    )
    ...
    latitude = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text='Worker location latitude'
    )
    ...
    longitude = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text='Worker location longitude'
    )

    working_radius_km = models.PositiveIntegerField(
        default=10,
        help_text='Working radius in kilometers'
    )

    resume = models.FileField(
        upload_to='worker_resumes/',
        blank=True,
        null=True
    )
    ...
    def extra_skills_list(self):
        return [
            s.strip()
            for s in self.extra_skills.split(',')
            if s.strip()
        ]
